	...
	with open(file + "/" + file + ".txt", "r") as f:
		...
		with open(file + "/" + file + "_train.csv", "w+") as output:
			with open(file + "/" + file + "_test.csv", "w+") as test:
				output.write("windSpdKts,gustKts,windVrbDeg,visibilityMtrs,qnh,ws")
				test.write("windSpdKts,gustKts,windVrbDeg,visibilityMtrs,qnh,ws")
				for line in data:
					...
					try:
						...
						if windGust is None:
							windGust = "0"
						text = "\n" + windSpd + "," + windGust + "," + matchWindVrb(filterData) + "," + matchVisibility(filterData) + "," + matchQnh(filterData) + "," + matchWs(filterData)
